...
1. Уменьшить том под __/__ до 8G
1. Выделить том под __/var__ ( __/var__ - сделать в mirror)
1. Выделить том под __/home__
1. Для __/home__ - сделать том для снэпшотов
...
1. Работа со снапшотами:
   - Сгенерировать файлы в __/home/__
   - Снять снэпшот
...
1. На куче дисков попробовать поставить btrfs/zfs:
    ...
    - Разметить здесь каталог __/opt__
...
### Уменьшить том под __/__ до 8G ###

- Устанавливаем дополнительно пакеты (xfsdump будет необходим для снятия копии тома):
```
...
-  Сымитируем текущий root -> сделав в него chroot и обновим grub:
```
...
- Изменяем конфиг grub, чтоб был смонтирован нужный  __/__, для этого в файле __/boot/grub2/grub.cfg__ меняем строку __rd.lvm.lv=VolGroup00/LogVol00__ на __rd.lvm.lv=vg_root/lv_root__

- Перезагружаемся и проверяем что новый __/__ на новом томе:
```
...
### Выделить том под __/var__ ( __/var__ - сделать в mirror) ###
...
- Создаем ФС и перемещаем туда __/var__:
```
...
- Монтируем новый var в каталог __/var__:
```
[root@lvm boot]# umount /mnt
[root@lvm boot]# mount /dev/vg_var/lv_var /var
```

- Правим fstab длā автоматического монтирования __/var__:
```
...
- Сгенерируем файлы в __/home/__:
```
